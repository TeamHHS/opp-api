...
async def process_payment(
    user: user_dependency, db: db_dependency, payment_request: PaymentRequest
):
    check_user_authentication(user)
    ...
    card_model = (
        db.query(Cards)
        .filter(Cards.card_number == payment_request.card_number)
    ...
    if (
        card_model.expiration_month != payment_request.expiration_month
        or card_model.expiration_year != payment_request.expiration_year
        or card_model.cvv != payment_request.cvv
        or card_model.card_type != payment_request.card_type
    ):
...
def update_payment_status(user: user_dependency, db: db_dependency):
    for payment_model in (
        db.query(Payments).filter(Payments.owner_id == user.get("user_id")).all()
    ):
